        ...
        return \Datatables::of($tickets)
                        ...
                        ->editColumn('title', function ($tickets) {
                            ...
                            if ($tickets->duedate != null) {
                                ...
                                if ($duedate - $now < 0) {
                                    $due = '&nbsp;<span style="background-color: #357ca5a(221, 75, 57, 0.67) !important" title="'.Lang::get('lang.is_overdue').'" class="label label-danger">'.Lang::get('lang.overdue').'</span>';
                                } else {
                                    if (date('Ymd', $duedate) == date('Ymd', $now)) {
                                        $due = '&nbsp;<span style="background-color: #357ca5a(240, 173, 78, 0.67) !important" title="'.Lang::get('lang.going-overdue-today').'" class="label label-warning">'.Lang::get('lang.duetoday').'</span>';
                                    }
                                }
                            }
                            ...
                            <a href='".route('ticket.thread', [$tickets->id])."'>".$string."&nbsp;<span style='color:#15c315'>".$thread_count."</span>
                            </a> <span><i style='color:#15c315' title='".Lang::get('lang.ticket_created_source', ['source' => $source])."' class='".$css."'></i></span>".$collabString.$attachString.$due.$tooltip_script.
                        ...
                        ->editColumn('a_uname', function ($tickets) {
                            if ($tickets->assigned_to == null && $tickets->name == null) {
                                ...
                                if ($tickets->assigned_to != null) {
                                    ...
                                    return "<a href='".$url."' title='".Lang::get('lang.see-profile1').' '.$assign.'&apos;'.Lang::get('lang.see-profile2')."'><span style='color:#15c315'>".mb_substr($assign, 0, 30, 'UTF-8').'</span></a>';
                                } else {
                                    $url1 = '#';

                                    return "<a href='".$url1."' title='".Lang::get('lang.see-profile1').' '.ucfirst($tickets->name).'&apos;'.Lang::get('lang.see-profile2')."'><span style='color:#15c315'>".mb_substr(ucfirst($tickets->name), 0, 30, 'UTF-8').'</span></a>';
                                }
                            }
                        })
    ...
    public static function getTable($tickets)
    {
        return \Datatables::of($tickets)
            ...
            ->addColumn('title', function ($tickets) {
                ...
                return "<div class='tooltip1' id='tool".$tickets->id."'>
                            <a href='".route('ticket.thread', [$tickets->id])."'>".ucfirst($string)."&nbsp;<span style='color:#15c315'>(".$tickets->countthread.") <i class='".$css."'></i></span>
            ...
            ->addColumn('assign_user_name', function ($tickets) {
                if ($tickets->assigned_to == null) {
                    ...
                    return "<a href='".$url."' title='".Lang::get('lang.see-profile1').' '.ucfirst($tickets->assign_first_name).'&apos;'.Lang::get('lang.see-profile2')."'><span style='color:#15c315'>".ucfirst($tickets->assign_first_name).' '.ucfirst($tickets->assign_last_name).'</span></a>';
                }
            })
