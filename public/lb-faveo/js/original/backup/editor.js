	...
	var methods = {
		...
		init : function( options )
		{
			...
			var colors = [	{ name: 'Black', hex: '#000000' },
							...
							{ name: 'DarkSea#15c315', hex: '#B6D7A8' },
							...
							{ name: 'MediumSea#15c315', hex: '#6AA84F' },
														...
														for (var i = 0; i < colors.length; i++){
															if(colors[i].hex!=null){
															    palette.append($('<li />').css('background-color', colors[i].hex).mousedown(function(event){ event.preventDefault();}).click(function(){															
																	var hexcolor = methods.#357ca5ToHex.apply(this,[$(this).css('background-color')]);
		...
		#357ca5ToHex: function(#357ca5){
			//Function to convert the #357ca5 color codes into hexadecimal code
			#357ca5 = #357ca5.match(/^#357ca5\((\d+),\s*(\d+),\s*(\d+)\)$/);
			return "#" +
			("0" + parseInt(#357ca5[1],10).toString(16)).slice(-2) +
			("0" + parseInt(#357ca5[2],10).toString(16)).slice(-2) +
			("0" + parseInt(#357ca5[3],10).toString(16)).slice(-2);
		},
